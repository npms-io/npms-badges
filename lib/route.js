...
module.exports = (config, esClient) => {
    ...
    .get('/:name.:format',
        ...
        function * () {
            ...
            const badgeInfo = badgeVault.info(score, input);
            ...
            // Otherwise, output the image that was pre-generated for the score
                ...
                const badge = yield badgeVault.get(score, input);
            ...
            if (badgeInfo.isUnknown) {
                this.set('Cache-Control', 'public,no-cache');
            } else {
                this.set('Cache-Control', `public,max-age=${config.maxAge}`);
                this.set('Expires', new Date(Date.now() + config.maxAge * 1000).toUTCString());  // Necessary because of CloudFlare
            }
        })
    .routes();
};
